...
class whiteAgent(CaptureAgent):
    ...
    """
        ...
        if self.defence:
            if nearestPacman:
                ...
                if len(self.lastupdate)>1:
                    self.escapepath = self.defAstar(gameState, self.mhtheuristic, self.lastupdate[1])
                    self.update = self.lastupdate[1]
                elif len(nearestOpponent)>0 and nearestOpponent[1]>3 and int(self.width/2)-5 < mypos[0] <int(self.width/2)+4:
            ...
            if not gameState.getAgentState(self.index).isPacman:
                ...
                elif len(nearestOpponent) > 0 and not nearestOpponent[3] and self.getMazeDistance(nearestOpponent[0],mypos)<4:
                ...
                if self.getPreviousObservation().hasFood(mypos[0], mypos[1]):
                    self.eaten += 1
                if self.isAnyPacman(gameState) and self.index>self.teamMate:
                    self.escapepath = self.astar(gameState, self.attackheuristic, nearestEscape, nearestEnemy)
                    self.defence = True
                    # print "PACMAN EATING, DEFENCE!!"
    ...
    def evaluate(self, gameState, nearestFood, nearestEnemy, escapepath, action, nearestOpponent):
        ...
        #         print self.index,"ATTACK TOO CLOSE TO GHOST"
        #         score -= 15
