...
class whiteAgent(CaptureAgent):
        ...
        if self.defence:
                ...
                if len(nearestPacman[0]) == 2:
                    ...
                    if (self.index > self.teamMate):
                        if not self.mateTarget == self.target:
                            ...
                            self.escapepath = self.defAstar(gameState, self.mhtheuristic, nearestPacman[0][0])
            elif len(nearestOpponent)>0 and nearestOpponent[1]>4 and int(self.width/2)-5 < mypos[0] <int(self.width/2)+4:
                # print mypos[0]
                # print self.index,"ATTACK!!!!"
                self.defence = False
            elif len(nearestOpponent) == 0 and int(self.width/2) - 5 < mypos[0] < int(self.width/2)+4:
                # print self.index,"ATTACK!!!!"
                self.defence = False
            else:
                if self.teamMate > self.index and int(self.width/2) - 5 < mypos[0] < int(self.width/2)+4:
                    self.defence = False
                else:
            ...
            if not gameState.getAgentState(self.index).isPacman:
                ...
                if self.getPreviousObservation().hasFood(mypos[0], mypos[1]):
                    self.eaten += 1
                if len(self.getAllPacman(gameState)[0])>0 and self.index>self.teamMate:
    ...
    def evaluateDefence(self,gameState, action, escapepath):
        ...
        score -= self.getMazeDistance(nextpos, self.s) * 10
        ...
        if len(invaders) == 0:
            prob = [self.inferenceMods[a].getMostLikelyPosition() for a in
                    self.inferenceMods]
            enemy_distance = [self.getMazeDistance(nextpos, self.inferenceMods[a].getMostLikelyPosition()) for a in
                              self.inferenceMods]
        ...
        if len(invaders) > 0:
            ...
            if len(pos) != 0:
                for i in range(len(pos)):
                    dists[i] = self.getMazeDistance(nextpos, pos[i])
                nearestDst = dists[0]
                ...
                for i in range(len(pro_pos)):
                    dists[i] = self.getMazeDistance(nextpos, pro_pos[i])
                nearestDst = dists[0]
                for i in range(len(dists)):
                    if dists[i] < nearestDst:
                        nearestDst = dists[i]
        ...
        for i in self.getOpponents(gameState):
            ...
            if pacman.isPacman:
                if pacman.getPosition() == None:
                    poslist.append(self.inferenceMods[i].getMostLikelyPosition())
                    dislist.append(self.getMazeDistance(myPos, self.inferenceMods[i].getMostLikelyPosition()))
                else:
